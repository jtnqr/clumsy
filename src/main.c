...
void init(int argc, char *argv[]) {
  ...
  topFrame =
      IupFrame(topVbox = IupVbox(
                   filterText = IupText(NULL),
                   controlHbox = IupHbox(
                       stateIcon = IupLabel(NULL),
                       filterButton = IupButton("Start", NULL),
                       hotkeyLabel = IupLabel(""), IupFill(),
                       IupLabel("Presets:  "), filterSelectList = IupList(NULL),
                       addPresetButton = IupButton("Save", NULL),
                       delPresetButton = IupButton("Delete", NULL), NULL),
                   NULL));
  ...
  // setup hotkey hint label (expands to fill space, text set when hotkey is
  // registered)
  ...
  IupSetAttribute(addPresetButton, "TIP",
  ...
  // set g_applying_preset = TRUE before loading parameters so sync callbacks
  // are not triggered to mark custom
  g_applying_preset = TRUE;

  // If state was loaded, restore the saved filter text
  if (stateLoaded) {
    const char *savedFilter = IupGetGlobal("filter");
    LOG("Restoring filter from state: %s",
  ...
  // Now we are done programmatically setting up the UI from loaded state or
  // defaults, so we set g_applying_preset = FALSE.
  g_applying_preset = FALSE;

  // Determine what preset selection to show
  if (stateLoaded) {
    const char *savedPreset = IupGetGlobal("preset");
    LOG("Restoring preset from state: %s",
    ...
    if (filtersSize > 0) {
      ...
      IupSetAttribute(filterSelectList, "VALUE",
                      "1"); // Select first preset (index 1)
    } else {
      char valBuf[32];
      snprintf(valBuf, sizeof(valBuf), "%d", filtersSize + 1);
      IupSetAttribute(filterSelectList, "VALUE", valBuf); // Select "<custom>"
  ...
  // dialog
  dialog = IupDialog(dialogVBox = IupVbox(topFrame, processFilterContainer,
                                          bottomFrame, footerBox, NULL));

  IupSetAttribute(dialog, "TITLE", "clumsy " CLUMSY_VERSION);
  IupSetAttribute(
      dialog, "SIZE",
      "540x"); // add padding manually to width (extra space for hotkey label)
  ...
  if (arg_value != NULL) {
    char valueBuf[16];
    snprintf(valueBuf, sizeof(valueBuf), "%s000",
             arg_value); // convert from seconds to milliseconds
